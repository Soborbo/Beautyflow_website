...
function validate(cfg) {
  ...
  // GA4 (OPTIONAL — omit for a migration site whose browser GA4 already fires via
  // GTM; the gateway then skips the GA4 MP leg so it isn't double-counted. Requires
  // the GA4-optional gateway, Soborbo/Serverside).
  if (cfg.ga4 === undefined || cfg.ga4 === null) {
    warn('ga4 omitted → the gateway will NOT send GA4 Measurement Protocol for this site (browser GA4 stays the single source). Correct for a migration site.');
  } else if (typeof cfg.ga4 !== 'object') {
    err('ga4 must be an object when present.');
  } else {
    ...
    if (!cfg.ga4.api_secret) err('ga4.api_secret is required when ga4 is present (MP api_secret from the GA4 admin UI).');
  }
...
function toSiteConfig(cfg) {
  ...
  // GA4 optional: only include when provided (migration sites omit it → gateway skips MP).
  if (cfg.ga4) sc.ga4 = { measurement_id: cfg.ga4.measurement_id, api_secret: cfg.ga4.api_secret };
