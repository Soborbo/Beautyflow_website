// src/lib/errors/catalogue.ts
// Teljes hibakód-regiszter — 290 generikus kód + projekt-specifikus kódok.
//
// Minden kód FIXÁLT severity + retryable + userImpact.
// requiredContext: ezek a context mezők KÖTELEZŐEK ehhez a kódhoz.
// Ha reportServerError()-t hívsz ezek nélkül, dev módban warning-ot kapsz.
//
// A weboldal (producer) a PROJECT_CODES kódjait emittálja; a generikus
// blokkok (HTTP/SRV/…/IMG) referencia-katalógusként szolgálnak, és a
// központi Tail Worker (soborbo-error-pipeline) is ezekből old fel severity-t.
...
export const HTTP_CODES: Record<string, ErrorCodeDef> = {
  ...
  'HTTP-401-001': { severity: 'WARN',     retryable: false, userImpact: 'blocked',  message: '401 Unauthorized — auth hiányzik', requiredContext: ['endpoint'] },
  'HTTP-401-002': { severity: 'WARN',     retryable: false, userImpact: 'blocked',  message: '401 — auth lejárt/érvénytelen', requiredContext: ['endpoint', 'keyPrefix'] },
  ...
  'HTTP-404-002': { severity: 'WARN',     retryable: false, userImpact: 'blocked',  message: '404 — API endpoint nem található', requiredContext: ['endpoint', 'method'] },
  ...
  'HTTP-500-001': { severity: 'ERROR',    retryable: true,  userImpact: 'blocked',  message: '500 Internal Server Error — saját endpoint', requiredContext: ['endpoint', 'errorMessage'] },
  'HTTP-500-002': { severity: 'ERROR',    retryable: true,  userImpact: 'blocked',  message: '500 — dependency threw', requiredContext: ['endpoint', 'dependency', 'depError'] },
  ...
  'HTTP-521-001': { severity: 'ERROR',    retryable: true,  userImpact: 'blocked',  message: '521 Web Server Is Down', requiredContext: ['cfRay'] },
  'HTTP-522-001': { severity: 'ERROR',    retryable: true,  userImpact: 'blocked',  message: '522 Connection Timed Out', requiredContext: ['cfRay', 'durationMs'] },
  'HTTP-524-001': { severity: 'ERROR',    retryable: false, userImpact: 'blocked',  message: '524 A Timeout Occurred — CF 30s limit', requiredContext: ['cfRay', 'endpoint'] },
};
...
export const SRV_CODES: Record<string, ErrorCodeDef> = {
  'SRV-FUNC-001':    { severity: 'ERROR',    retryable: false, userImpact: 'blocked',  message: 'Worker unhandled exception', requiredContext: ['functionPath', 'errorMessage'] },
  'SRV-FUNC-002':    { severity: 'ERROR',    retryable: false, userImpact: 'blocked',  message: 'Worker timeout (>30s)', requiredContext: ['functionPath', 'durationMs', 'lastStep'] },
  ...
  'SRV-MW-001':      { severity: 'ERROR',    retryable: false, userImpact: 'blocked',  message: 'Middleware threw', requiredContext: ['errorMessage'] },
...
export const RESEND_CODES: Record<string, ErrorCodeDef> = {
  ...
  // Generic primary-provider send failure. Producers emit this on any
  // non-recoverable Resend send error before falling back to Brevo. ERROR
  // (not CRITICAL) — only EMAIL-BOTH-001 fires CRITICAL once both fail.
  'RESEND-SEND-001':  { severity: 'ERROR',    retryable: true,  userImpact: 'degraded', message: 'Resend send failed (primary)', requiredContext: ['statusCode', 'attempt'] },
};
...
export const FORM_CODES: Record<string, ErrorCodeDef> = {
  ...
  'FORM-SUBMIT-001':   { severity: 'ERROR',    retryable: false, userImpact: 'blocked', message: 'Handler top-level exception', requiredContext: ['formId', 'errorMessage'] },
  'FORM-SUBMIT-002':   { severity: 'ERROR',    retryable: false, userImpact: 'blocked', message: 'Handler timeout (CF 30s)', requiredContext: ['formId', 'durationMs'] },
...
export const CALC_CODES: Record<string, ErrorCodeDef> = {
  'CALC-PRICE-001':    { severity: 'ERROR',    retryable: false, userImpact: 'blocked',  message: 'Pricing function throw', requiredContext: ['answers', 'errorMessage'] },
...
export const NET_CODES: Record<string, ErrorCodeDef> = {
  ...
  'NET-DNS-001':      { severity: 'WARN',  retryable: true,  userImpact: 'blocked',  message: 'DNS resolution failed', requiredContext: ['hostname'] },
  'NET-SSL-001':      { severity: 'WARN',  retryable: false, userImpact: 'blocked',  message: 'SSL/TLS hiba', requiredContext: ['hostname'] },
  ...
  // Sub-resource load failure (script/link/source/video/audio). IMG has its
  // own dedicated code (IMG-LOAD-001).
  'NET-RESOURCE-001': { severity: 'WARN',  retryable: true,  userImpact: 'degraded', message: 'Sub-resource failed to load', requiredContext: ['src', 'tag'] },
};

export const JS_CODES: Record<string, ErrorCodeDef> = {
  // JS errors are typically browser/extension noise (ad blockers, old devices,
  // injected userscripts). Default WARN; promote to ERROR/CRITICAL via rate
  // aggregation outside this catalogue, not per-event.
  'JS-UNHANDLED-001': { severity: 'WARN',  retryable: false, userImpact: 'degraded', message: 'Uncaught synchronous error', requiredContext: ['source', 'line', 'col'] },
  'JS-PROMISE-001':   { severity: 'WARN',  retryable: false, userImpact: 'degraded', message: 'Unhandled promise rejection', requiredContext: ['message'] },
  'JS-SYNTAX-001':    { severity: 'ERROR', retryable: false, userImpact: 'blocked',  message: 'Script parse error', requiredContext: ['source'] },
  'JS-TYPE-001':      { severity: 'WARN',  retryable: false, userImpact: 'degraded', message: 'TypeError', requiredContext: ['source', 'message'] },
  'JS-REF-001':       { severity: 'WARN',  retryable: false, userImpact: 'degraded', message: 'ReferenceError', requiredContext: ['source', 'message'] },
  'JS-RANGE-001':     { severity: 'WARN',  retryable: false, userImpact: 'blocked',  message: 'RangeError', requiredContext: ['source'] },
};
...
// ============================================================
// PROJECT-SPECIFIC — beautyflow-website kódok
// ============================================================
//
// Ezek a kódok a weboldal által ténylegesen emittált kódok (lásd ERROR_CODES
// a codes.ts-ben). A requiredContext mezők a tényleges reportServerError()
// hívásokhoz igazítva — dev módban warning, ha hiányoznak.

export const PROJECT_CODES: Record<string, ErrorCodeDef> = {
  // ---- /api/contact -----------------------------------------------------
  'CONTACT-EMAIL-ADMIN-001':     { severity: 'ERROR',    retryable: true,  userImpact: 'degraded', message: 'Resend admin értesítő email elutasítva', requiredContext: ['formType'] },
  'CONTACT-EMAIL-USER-001':      { severity: 'WARN',     retryable: true,  userImpact: 'degraded', message: 'Resend visszaigazoló email elutasítva (admin megkapta a leadet)', requiredContext: ['formType'] },
  'CONTACT-EMAIL-BOTH-001':      { severity: 'CRITICAL', retryable: false, userImpact: 'blocked',  message: 'Admin ÉS user email is failed — lead elveszhet', requiredContext: ['formType', 'adminReason', 'userReason', 'lead'] },
  'CONTACT-SHEETS-001':          { severity: 'WARN',     retryable: true,  userImpact: 'none',     message: 'Google Sheets append failed (lead emailben + logban megvan)', requiredContext: ['formType', 'lead'] },
  'CONTACT-CONFIG-RESEND-001':   { severity: 'CRITICAL', retryable: false, userImpact: 'blocked',  message: 'RESEND_API_KEY nincs beállítva — minden submit 500', requiredContext: [] },
  'CONTACT-CONFIG-TURNSTILE-001':{ severity: 'CRITICAL', retryable: false, userImpact: 'blocked',  message: 'TURNSTILE_SECRET_KEY nincs beállítva — submitek elutasítva (fail closed)', requiredContext: [] },
  'CONTACT-TURNSTILE-001':       { severity: 'WARN',     retryable: true,  userImpact: 'degraded', message: 'Turnstile elutasított egy submitet (bot vagy lejárt token)', requiredContext: ['formType', 'errors'] },
  'CONTACT-UNHANDLED-001':       { severity: 'ERROR',    retryable: false, userImpact: 'blocked',  message: 'Top-level catch a contact POST handlerben', requiredContext: [] },

  // ---- /api/boranalizis (bőranalízis kvíz) ------------------------------
  'QUIZ-CONFIG-TURNSTILE-001':   { severity: 'CRITICAL', retryable: false, userImpact: 'blocked',  message: 'TURNSTILE_SECRET_KEY nincs beállítva — quiz submitek elutasítva', requiredContext: [] },
  'QUIZ-TURNSTILE-001':          { severity: 'WARN',     retryable: true,  userImpact: 'degraded', message: 'Turnstile elutasított egy quiz submitet', requiredContext: ['errors'] },
  'QUIZ-CONFIG-RESEND-001':      { severity: 'CRITICAL', retryable: false, userImpact: 'blocked',  message: 'RESEND_API_KEY nincs beállítva — quiz submitek 500', requiredContext: [] },
  'QUIZ-EMAIL-BOTH-001':         { severity: 'CRITICAL', retryable: false, userImpact: 'blocked',  message: 'Szalon ÉS user email is failed — quiz lead elveszhet', requiredContext: ['lead', 'adminReason', 'userReason'] },
  'QUIZ-EMAIL-ADMIN-001':        { severity: 'ERROR',    retryable: true,  userImpact: 'degraded', message: 'Szalon értesítő email failed (user megkapta a leadet + eredményt)', requiredContext: [] },
  'QUIZ-EMAIL-USER-001':         { severity: 'WARN',     retryable: true,  userImpact: 'degraded', message: 'User visszaigazoló/eredmény email failed (szalon értesítve)', requiredContext: [] },
  'QUIZ-SHEETS-001':             { severity: 'WARN',     retryable: true,  userImpact: 'none',     message: 'Google Sheets append failed (lead emailben + logban megvan)', requiredContext: ['lead'] },
  ...
  'TRACK-GA4-ABANDON-001':       { severity: 'WARN',     retryable: true,  userImpact: 'none',     message: 'GA4 Measurement Protocol abandonment beacon nem továbbítódott', requiredContext: [] },
};
...
const GLOBAL_MAPS: Record<string, ErrorCodeDef>[] = [
  HTTP_CODES,
  SRV_CODES,
  RESEND_CODES,
  BREVO_CODES,
...
// Module-load sanity: warn if PROJECT_CODES silently shadows a global code.
// We don't throw — that would brick the worker on a typo — but the warning
// shows up in `wrangler tail` and the admin's logs for early detection.
for (const projectKey of Object.keys(PROJECT_CODES)) {
  for (const map of GLOBAL_MAPS) {
    if (Object.prototype.hasOwnProperty.call(map, projectKey)) {
      console.warn(
        `[error-pipeline] PROJECT_CODES['${projectKey}'] shadows a global code definition. ` +
          `Rename the project code to avoid silent override.`,
      );
    }
  }
}

export const ALL_CODES: Record<string, ErrorCodeDef> = Object.assign(
  Object.create(null),
  ...GLOBAL_MAPS,
  PROJECT_CODES,
);

/** Look up a code's definition, or undefined if it isn't catalogued. */
export function getErrorCodeDef(code: string): ErrorCodeDef | undefined {
